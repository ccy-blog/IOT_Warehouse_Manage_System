...
void ADC_Collect(uint32_t *adc)
{
  ...
  ADCInit(ADC_CLK);
	...
	*adc = adv; 						  //传参
 	snprintf(buf, 16, "adv: %d  ", adv);
	OLED_DisStrLine(6, 0, (uint8_t *)buf);//OLED屏显示
}
void Bet_Collect(uint32_t * bet)
{
...
//		batsum = 0;
//		}
		snprintf(buf, 16, "bet: %d ", *bet);
		OLED_DisStrLine(7, 0, (uint8_t *)buf);	 //OLED屏显示
